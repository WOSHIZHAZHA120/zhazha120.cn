	...
	<div class="my-10">
		<Transition appear mode="out-in" name="fade">
			<template v-if="data.length <= 0">
				<tools-accuracy_calculator-results-empty/>
			</template>
					...
					<template v-if="results.length > 0">
						<div class="mt-10">
							<div class="flex flex-col gap-2">
								...
								<tools-accuracy_calculator-results-can_pass :data="data" :pass-require-percentage="passRequirePercentage" :results="results"/>
							</div>
						</div>
					</template>
				</div>
